...
Follow the extracted API's uses, !include, traits, resourceTypes and type/example references relative to their defining documents. Missing from the root ZIP means search the LOCAL cache: read the actual reference/POM metadata for that dependency's own group/artifact/version/entry, probe its exact repository directory and archive entries, then extract it separately. Preserve wrapper paths and transitive references. Reuse verified documents; maintain active-reference cycle state. Check existing source/Exchange paths and a narrow repository index for exact referenced identity before declaring missing; never substitute another version or scan unrelated projects.

For each method, follow its ACTUAL `is` entries through uses aliases and parameters to the referenced trait definitions, including those in locally cached `csp-dhisp-common-api-library.raml` when applied. Read selected request headers, not every header in that library. Merge explicit method headers and applicable resource/resourceType/trait/security declarations using the actual RAML version's precedence. `is: []` adds no method traits; it does not cancel resource-applied traits. Resource traits do not automatically propagate to nested resources. Resolve included/nested/parameterized traits and named-example value wrappers. Exclude response headers and unapplied traits.

Record `method/path | main API evidence | actual is entries | alias/library/trait chain | effective required headers | valid value/type/enum evidence`. Obtain values from valid examples/defaults/enums or existing test-property expressions; never invent credentials/business values or print secrets. Every execution http:request has an explicit canonical http:headers map and `doc:name="Request to <actual request path>"`, including error tests.

If the main API exists but a fragment remains unavailable AFTER local searches, use resolved RAML plus current operation-matched tests/fixtures and actual XML/DWL/property evidence. Record missing references and current source provenance; keep known required headers. Mark affected contracts source-backed and every affected suite with `<!-- RAML-CONTRACT: source-backed; required-header completeness is unverified. -->`. Continue evidence-backed generation without a menu, reporting RAML validation pending. Never claim unknown constraints resolved, an observed empty header set complete, or fabricate essential values. Restored dependencies trigger narrow reconciliation and removal of markers only after full resolution.
...
Independently read every source error-handler declaration and its callers/refs/defaults before suites. Track exact file/location, type list, when, first-match order, body branches and concrete trigger. Include handlers without doc:id; account for unowned/out-of-scope declarations. Reachable APIKit handlers REQUIRE apikit-error-test-suite.xml; reachable main/business handlers REQUIRE error-test-suite.xml; local try handlers stay in their endpoint suite. Use actual names even when misspelled in source. Cover each comma-separated matcher type, feasible when/body alternative and actual error propagation; ANY is a matcher, never a thrown type. `errors` discovers anchor operations but requires only applicable error suites. Never finish after endpoint suites while handler owners remain pending.
...
For Mode B use this router error injection with exact selectors; Mode C uses actual backend triggers instead. Declare the prefix of the selected source processor only when suite XML actually uses it.
...
Run only the appropriate OS blocks at the RAML/dependency steps. Resolve the main archive from POM first; these commands do not choose an artifact. Use literal quoting (PowerShell doubles embedded single quotes; shell uses standard single-quote escaping). Verify temp resolves outside the application. Missing/corrupt/unsafe archives fail explicitly; never promise extraction cannot fail.
