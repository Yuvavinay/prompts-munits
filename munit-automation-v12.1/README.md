# MUnit automation — version 12.1.0

Latest update: mandatory local common-library header recovery, guarded external-temp extraction and explicit model-neutral tool requirements. The simple router and the compact worker structure are retained.
...
For another session, take MUNIT-AUTOMATION-CONTEXT.md together with the three prompt files from the handoff ZIP. A Claude chat without filesystem/terminal tools can review the package; executing it requires a suitable agent environment. No old session or hidden helper files are needed.
...
function Install-MUnitCommands {
    ...
    foreach ($name in @('munit-http-listener','munit-non-http-listener','munit-generate')) {
        ...
        if (-not $parts.Success -or $parts.Groups['header'].Value -notmatch "(?m)^name: '$name'\r?$" -or $parts.Groups['header'].Value -notmatch "(?m)^version: '12\.1\.0'\r?$") { throw "Invalid bundled metadata: $source" }
        if ($Format -eq 'Skill') {
            ...
            $content="---`nname: '$name'`n$description`nmetadata:`n  version: '12.1.0'`n---`n"+$body
...
for name in "${names[@]}"; do
    ...
    /usr/bin/awk -v expected="$name" '
      ...
      !closed && $0 == "version: '\''12.1.0'\''" {version_ok=1}
