...
`is: [...]` names applied traits; it is not an empty-header declaration. For EVERY method/path, including every error-test anchor, follow each applied trait to its definition. Resolve resource-level and method-level `is`, nested traits, `uses` library aliases in the defining document, `!include` paths, parameterized traits and the resourceType chain. Substitute supplied/reserved parameters before reading header names/values. An unresolved alias, include, parameter or cycle blocks the run before project writes. Never scan only the inline method `headers:` or copy the trait name as a header.
