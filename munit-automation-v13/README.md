# MUnit automation — version 13.0.0

Latest update: complete error-only scope, source-derived assertions including valid null results, concrete non-HTTP execution examples, and an executable completion/checkpoint gate. POM-selected RAML, applied-trait library recovery, external-temp extraction and the small router are retained.
...
The HTTP worker selects the MAIN RAML artifact from pom.xml and the APIKit reference, locates its exact coordinates in the effective local Maven repository, and extracts it. The known csp-dhisp-common-api-library.raml filename never selects or replaces that artifact. After extraction, the worker follows each method's actual is/uses references into locally cached trait libraries to resolve effective request headers. Only applied traits and inherited/explicit declarations contribute headers. If required headers or applied references are unresolved, the worker MUST locate the expected csp-dhisp-common-api-library.raml in the local repository, including cached ZIP entries, extract it to external temp and follow the actual method is/uses chain before accepting empty/pending headers. It verifies availability; it does not install missing content or change .m2. The extraction commands reject temp destinations inside the application, Maven repository or user .m2 directory.
...
## Model portability and handoff

The same instructions are intended for Claude, GPT and Gemini models when the configured agent host exposes the needed file/search/edit/terminal tools. There are no model pins, provider-specific API calls or provider-based workflow branches. Auto remains a host selection. Subagents are optional; the serial workflow has the same requirements. This is model-neutral design, not a completed benchmark or a guarantee for every model/version. Tool access, permissions and the provisioned Mule toolchain remain prerequisites.

For another session, take MUNIT-AUTOMATION-CONTEXT.md together with the three prompt files from the handoff ZIP. A Claude chat without filesystem/terminal tools can review the package; executing it requires a suitable agent environment. No old session or hidden helper files are needed. Checkpoints are optional reusable run state in external temp, not a dependency; when unavailable or stale the worker rebuilds its plan from source.
...
This is an instruction-driven workflow, not a complete deterministic RAML/DWL compiler. Independent source discovery still establishes the required scenarios. The embedded completion gate compares those declared obligations with actual staged files and current source hashes; it does not compile RAML/DWL or prove the source inventory is complete. Reconciliation and publication checks preserve project boundaries. Do not claim equivalent automated enforcement or reuse old v11 validation counts. All scripts are contained in the worker Markdown; no external helper files, model pinning or new dependencies are required.
...
See [VALIDATION.md](VALIDATION.md). This revision was checked using platform-neutral Python authoring/structural checks only. No macOS-specific command, PowerShell, installer, VS Code or Mule command was executed for this revision. The extraction commands now include a temp-location guard; their text/ordering and the other examples were checked without platform execution. This is not a fresh native-command, cache-availability or model benchmark.
...
function Install-MUnitCommands {
    ...
    foreach ($name in @('munit-http-listener','munit-non-http-listener','munit-generate')) {
        ...
        if (-not $parts.Success -or $parts.Groups['header'].Value -notmatch "(?m)^name: '$name'\r?$" -or $parts.Groups['header'].Value -notmatch "(?m)^version: '13\.0\.0'\r?$") { throw "Invalid bundled metadata: $source" }
        if ($Format -eq 'Skill') {
            ...
            $content="---`nname: '$name'`n$description`nmetadata:`n  version: '13.0.0'`n---`n"+$body
...
for name in "${names[@]}"; do
    ...
    /usr/bin/awk -v expected="$name" '
      ...
      !closed && $0 == "version: '\''13.0.0'\''" {version_ok=1}
