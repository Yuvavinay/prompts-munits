...
Discover all main/local handlers independently through declarations, named/default/reusable refs and real callers. Cover each comma-separated type, when/first-match condition and body branch with a concrete registered trigger; ANY is a matcher, not a thrown error. Main/business scenarios own error-test-suite.xml; inline try scenarios stay with the entry. `errors` discovers anchor entries and selects every applicable error scenario: main/business, reusable and local/inline try handlers. Global cases retain error-test-suite.xml; local cases retain their entry suite. Select error cases and necessary setup only, preserving ordinary success tests unchanged. Account for unreachable/unowned declarations; preserve unrelated HTTP-owned tests in an existing shared suite. Do not generate APIKit error suites or import REST Mode A/B/C rules here.
...
Replace all markers and use lowercase filenames. in/ roles are request-example or scenario-input; the out/ role is backend-output. Constraints is mandatory; [] only when no applicable facts exist. Compile ALL known effective required/type/enum facts, preserving source evidence; types are string, number, integer, boolean, null, object or array. JSON Pointer uses concrete array indices and ~0/~1 escapes; strings/property names are case-sensitive and object enum order is irrelevant. Missing optional fields skip checks; model optional parents correctly. Validate all broader RAML/DWL facets/predicates separately.
