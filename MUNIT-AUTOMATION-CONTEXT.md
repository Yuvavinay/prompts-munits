...
**Current implementation:** Version 6.0.0  
...
Version 6 is the latest delivered package. Continue from its actual files; do not reconstruct them from this summary or revert to an older scaffold prompt. This context records requirements and decisions. It is not a request to install anything, run application tests, or rewrite the package automatically: follow the user's next task.

The user’s main unresolved concern is whether the agent really drills into every relevant flow/sub-flow and generates the corresponding scenarios, instead of producing one generic test per suite. Version 6 adds stronger executable checks, but its success on a real application has **not** been demonstrated.
...
## Existing tests and version 6’s executable gate
...
Version 6 uses a frozen OS-temp plan.json with:
...
- Each planned test has `name`, `covers`, `inputs` and `mocks`.
...
The three version 6 prompts total 94,547 UTF-8 bytes versus 117,578 in version 5: **19.6% smaller by file size**. This is not a token benchmark.
...
1. Read all three version 6 prompts and VALIDATION.md before editing. Keep requirements fixed unless the user changes them.
...
```text
Read MUNIT-AUTOMATION-CONTEXT.md and the attached munit-automation-v6
Markdown files. Continue from version 6, preserving the recorded requirements.
...
per suite. Distinguish tested plan/XML checks from unverified Mule runtime results.
