...
If dependency content is still missing after those checks, stop before project writes with ONE actionable diagnostic:

```text
RAML dependency unresolved after local search. The main API RAML was found.
Reference: <literal uses/include value> from <actual referring file>
Dependency: <exact evidenced coordinates and entry, or unresolved identity>
Checked: <actual absolute paths/archive entries and command results>
State: <missing-after-local-search | unreadable | ambiguous | invalid-content>
Needed: <the exact missing content, access, or identity clarification>
No project files were written. Resume from the saved dependency ledger after this is resolved.
```

Do not report “RAML missing” when the root exists. Do not ask for the entire RAML again, fabricate required headers, relax header validation, download dependencies, or substitute another version. If the needed library is truly unavailable, stopping is correct regardless of the selected model. Another run's apparent success is not evidence of the missing library's actual contents.
...
`is: [...]` names applied traits; it is not an empty-header declaration. For EVERY method/path, including every error-test anchor, follow each applied trait to its definition. Resolve resource-level and method-level `is`, nested traits, `uses` library aliases in the defining document, `!include` paths, parameterized traits and the resourceType chain. Substitute supplied/reserved parameters before reading header names/values. A missing referenced file first requires the complete local dependency procedure above. Only a reference still unresolved after that procedure, or an invalid parameter/cycle, blocks the run before project writes. Never scan only the inline method `headers:` or copy the trait name as a header.
...
Use a valid resolved RAML example/default/enum example or an existing test-property expression with a proven key for each required header. Resolve named-example value wrappers. Validate constraints and serialization; never invent client credentials, tokens or business values. If evidence supplies no valid value, stop with `Required RAML header value unresolved: <method> <path> / <header>; source <trait/file>. Supply a valid test value.` Do not print secret values. Do not add production secrets to fixtures. JSON data files hold bodies/backend results; headers belong in the HTTP request's header map.
...
- Exactly one direct `<http:headers>` child containing a DataWeave object with ALL effective required request headers and any additional source-evidenced scenario headers. Emit even `{}` when the effective set is genuinely empty. No comment/placeholder can substitute for the map. Do not rely solely on client defaults.
