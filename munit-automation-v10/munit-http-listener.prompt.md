...
## Plan all owners, batch fixtures, then write every suite

Freeze the complete endpoint/handler inventory first. Keep `owner | planned scenarios | ready scenarios | next action | contract status | gaps` in temp. Recursively plan each owner's complete graph and DWL data origins, saving compact results before moving to the next. Resolve effective request headers and collect every selected RAML request example; then finish all branch/backend fixture variants and run the complete fixture batch. Only then construct XML for each endpoint followed by every required APIKit/main handler suite. Reuse definition reads, never coverage contexts; do not stop after one owner or return a planning summary.

Print `→ Planning <suite>`, the call/branch counts, `→ Staging all JSON fixtures (<N>)`, then `→ Building <suite> (<N> scenarios; contract <resolved|source-backed>)`. Update the next-action ledger after each phase and owner. Run the native checks against the full projected tree before narrow publication; source-backed contract status still allows that gate and generation. Unresolved essential case facts remain precise gaps, never fabricated readiness.
