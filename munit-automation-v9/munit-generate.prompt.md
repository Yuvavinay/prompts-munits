...
For genuine mixed scope, HTTP PLAN runs first, including recursive local RAML dependency resolution, then non-HTTP PLAN. A missing path inside the main RAML ZIP requires the HTTP worker's local dependency search, not immediate termination. If the main API exists but a fragment remains unavailable AFTER that search, the HTTP worker continues with explicit source-backed contract status using available source/test evidence. Do not treat that missing fragment as a blanket write blocker or offer a non-HTTP alternative. A missing root API or essential case-specific unknown remains a distinct blocker. Merge shared test-config/properties and handler-scenario ownership into one complete plan before APPLY; do not validate a partial worker plan as if it represented all module handlers. Never overwrite another entry's error tests.

6. Require the source-handler/request/scenario/mock checks. Process one endpoint at a time using a temp next-action ledger, then every required APIKit/main handler suite. Continue through every required owner and planned scenario; distinguish files generated from pending RAML/runtime validation. A per-suite Done line does not finish the invocation. Final status includes ready/required suites, scenarios and mock bindings, runtime/coverage evidence and any precise blocker.

## Continue unless an actual blocker remains

Do not finish with a plan summary, “Next: ...”, “Would you like me to proceed?”, or options to show mappings/run unrelated non-HTTP work when the requested scope can continue. Tool results drive the next action. Mark completed only when a step's exit conditions pass; “HTTP planning completed (partial)” is contradictory. If a required worker/tool is unavailable, source is ambiguous, access fails or the root contract itself is missing, report the exact blocker and missing evidence. Exhausted library lookup with the main API present triggers source-backed generation in the worker, not a stop menu. Never fabricate values to avoid stopping.
